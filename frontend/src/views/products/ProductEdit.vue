<template>
  <div class="product-edit-container">
    ...
    <el-card v-if="productForm.id" shadow="hover">
      <el-form
        ref="productFormRef"
        :model="productForm"
        :rules="rules"
    ...
    <el-empty v-else :description="loading ? '加载中...' : '产品不存在'" />
  </div>
</template>
  ...
  setup() {
    ...
    const fetchProductDetails = async (productId) => {
      ...
      try {
        ...
        const errorMessage = error.response?.data?.message || error.message || '获取产品信息失败'
    ...
    const handleSubmit = async () => {
      if (!productFormRef.value || !productForm.id) return
      ...
      try {
        ...
        // 更详细的错误处理
        const errorMessage = error.response?.data?.message || error.message || '产品更新失败'
        ElMessage.error(errorMessage)
    ...
    const handleReset = () => {
      if (productFormRef.value) {
        fetchProductDetails(productForm.id)
